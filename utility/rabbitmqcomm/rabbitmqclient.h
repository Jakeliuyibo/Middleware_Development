...
namespace utility
{
    ...
    class RabbitMqClient
    {
        ...
        public:
            ...
            // 订阅方式被动获取一条消息
            int consume(const string &queue_name, string &message, struct timeval *timeout);
            // 订阅方式被动获取多条消息
            int consume(const string &queue_name, vector<string> &message, int num, struct timeval *timeout);
            // 主动获取消息
            int read(const string &queue_name, string &message);
        
        private:
            // 处理错误信息
            int errorMsg(const amqp_rpc_reply_t &reply, const string & desc);   
            // 消费者被动接收进行消费
            int consumeMessage(const string queue_name, vector<string> &message, int num, struct timeval *timeout, bool no_ack=true);
            // 消费者主动拉去消息
